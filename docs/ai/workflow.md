...
- [ ] 아래 결과 보고 형식에 따라 실제 실행 명령과 각 결과를 함께 제출했는가?
...
최종 응답과 PR 본문에는 **실제로 실행한 검증 명령과 해당 결과를 짝지어** 제출한다. "테스트 통과", "CI 성공"처럼 통과 여부만 적는 것으로는 완료 보고를 대신할 수 없다.

- 명령은 실행한 옵션과 대상까지 그대로 적는다. `yarn verify`를 실행했다면 내부의 lint·typecheck·test·build 결과도 구분한다. 중간 실패로 실행되지 않은 단계는 통과로 표시하지 않는다.
- 결과에는 `통과 / 실패 / 미실행`과 실제 출력의 핵심 내용을 기록한다. 테스트는 파일·테스트 개수, 린트는 오류·경고, 빌드는 완료 여부와 주요 경고를 포함한다. 숫자는 출력에서 확인된 경우에만 적는다.
- 실패 시 핵심 오류와 조치, 재실행한 명령 및 최종 결과를 적는다. 미실행 시 이유를 명시한다. 문서만 변경해 앱 검증을 생략한 경우도 동일하다.
- 로컬 작업 폴더, 별도 작업 사본, CI 결과를 구분한다. 서로 다른 코드 상태를 검증했다면 브랜치·커밋 등 식별 정보를 함께 적는다. CI는 해당 실행 또는 PR 검사 링크를 첨부한다.
- 로그 전체를 붙일 필요는 없지만 결과를 확인할 수 있는 출력 요약이나 로그 링크를 제공한다. 비밀값은 포함하지 않는다.

제출 형식 예시(자리표시자는 실제 결과로 바꾸며, 실행하지 않은 명령은 미실행으로 표시):

| 실행 환경 | 실행 명령 | 결과 | 출력 요약 또는 근거 |
|---|---|---|---|
| 로컬 작업 폴더 | `yarn test:run` | 통과/실패/미실행 | 테스트 파일·테스트 개수 또는 실패 원인 |
| 로컬 작업 폴더 | `yarn lint` | 통과/실패/미실행 | 오류·경고 또는 미실행 이유 |
| 로컬 작업 폴더 | `yarn typecheck` | 통과/실패/미실행 | 종료 코드 또는 타입 오류 |
| 로컬 작업 폴더 | `yarn build` | 통과/실패/미실행 | 빌드 완료 여부와 주요 경고 |
| CI (커밋 식별자) | 실제 검사 명령 | 통과/실패/미실행 | 실행 또는 PR 검사 링크 |

Storybook 관련 변경이 있으면 `yarn build-storybook`도 같은 형식으로 제출한다. 이 보고 형식은 기존 필수 검증과 병합 조건을 대체하거나 완화하지 않는다.
...
`src/pages/Word/studyFlow.test.tsx`는 실제 Home/Word 페이지, 라우터, React Query, Zustand, 공통 모달을 조합하고 API 응답만 고정해 학습 흐름을 검증한다.

- 단어 답변은 즉시 영속 저장하며, 현재 회독의 목록은 저장 상태와 분리해 단어가 건너뛰어지지 않게 한다.
- 완료 여부는 이번 답변까지 반영한 복습 목록이 비었는지로 판단한다. 마지막 단원의 실제 단어 수도 고려한다.
- 학습 시간은 초 단위 변경분만 누적한다. 종료 시 전체 시간을 다시 더하지 않는다. 1초 미만 시간은 기록하지 않는다.
- 최근 학습 위치는 `latestStudy`를 정본으로 사용한다. 기존 `lastStudy`는 레벨별 위치를 보존한다. 이전 데이터에 최근 위치가 없고 레벨이 하나면 이어하기를 제공하며, 여러 레벨이면 다음 학습까지 순서를 추측하지 않는다.
- StrictMode, 저장 후 재수화, 브라우저 뒤로가기, 단원 이동, 복습과 재회독, 홈 이어하기 및 AI 분석 입력을 검증한다.

이 테스트는 jsdom 통합 테스트이며 실제 브라우저의 강제 종료·저장소 삭제·다중 탭 충돌까지 보장하지 않는다.
