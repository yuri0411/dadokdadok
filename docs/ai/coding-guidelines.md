...
- 확인/취소용 `Modal`은 `title` 또는 `aria-label`로 접근 가능한 이름과 `role="dialog"` / `aria-modal`을 제공한다. 열릴 때 포커스를 내부로 옮기고 Tab/Shift+Tab을 내부에서 순환시키며, 배경을 `inert`로 비활성화하고 닫을 때 이전 포커스를 복원한다. Escape 동작은 닫기 설정을 따른다.
